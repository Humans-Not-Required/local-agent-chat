...
To get the N most recent messages without knowing the current seq, use `?latest=N`:
```
GET /api/v1/rooms/{room_id}/messages?latest=10
```
Returns the 10 most recent messages in chronological order. Combines with `sender` and `sender_type` filters.
