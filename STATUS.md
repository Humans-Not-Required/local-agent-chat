...
- [x] **Consistent JSON error responses** — `read_positions.rs` was the only route file returning bare `Status` errors; now returns `(Status, Json)` with descriptive messages matching all other routes. 8 new tests: negative seq validation, sender length validation, JSON error body verification, llms.txt section coverage, OpenAPI path count, health version field, SPA fallback. 374 total tests. Commit: 0f87a74.
